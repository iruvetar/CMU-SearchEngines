...
public class InvList {
  ...
  public int getTfByDocId(int docid) {
     for (int i = 0; i < this.df; i++) {
         if(this.postings.get(i).docid == docid) {
             return this.postings.get(i).tf;
         }
     }
     return 0; //no such document with this docid exists in the postings
  }
  
  /**
   * Return the location list of the term in the docIndex'th document
   * @return Vector<Intger> a list of positions
   */
  public Vector<Integer> getLoc(int docIndex) {
      return this.postings.get(docIndex).positions;
  }
