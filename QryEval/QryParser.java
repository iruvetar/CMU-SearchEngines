...
public class QryParser {
   ...
   */
  private static Qry createOperator (String operatorName, RetrievalModel model) {
    ...
    //  #near/n and #window/n.
    if (operatorNameLowerCase.indexOf('/') != -1) {
        String[] operatorN = operatorNameLowerCase.split("/");
        operatorNameLowerCase = operatorN[0];
        int distance = Integer.parseInt(operatorN[1]);
        ...
        operator.setDisplayName (operatorNameLowerCase);
        return operator;
    }       
   ...
   */
  public static Qry getQuery (String queryString, RetrievalModel model)
      throws IOException, IllegalArgumentException {

    Qry q = parseString (queryString, model);		// An exact parse
   ...
   *  @throws IllegalArgumentException Query syntax error.
   */
  private static Qry parseString (String queryString, RetrievalModel model)
      throws IOException, IllegalArgumentException {
    ...
    String[] substrings = queryString.split("[(]", 2); // split by the left (
    Qry queryTree = createOperator (substrings[0].trim(), model);
    ...
    while (queryString.length() > 0) {
      ...
      //  STUDENT HW2 CODES
      //  Handle the weights
      if(queryTree.getDisplayName().equalsIgnoreCase("#wand") || queryTree.getDisplayName().equalsIgnoreCase("#wsum")) {
	  ...
	  p = popSubquery (queryString); //return the first query and remainig string
	  qargs = new Qry[1];
	  qargs[0] = parseString (p.getPopped(), model); //Getpopped will return the first query, parseString conduct recursive call
      } else {                        // For term
	  p = popTerm (queryString); //Split term and the remaining string by space
	  qargs = createTerms (p.getPopped()); //Getpopped return the first element
      }
      ...
      //  Add the argument(s) to the query tree.
          ...
          for (int i=0; i<qargs.length; i++) {
        	  ...
        	  if(queryTree.getDisplayName().equalsIgnoreCase("#wand") || queryTree.getDisplayName().equalsIgnoreCase("#wsum")) {
            	  queryTree.appendWeights(weight);
        	  }	  
        		queryTree.appendArg (qargs[i]); //use this term to create QryIopTerm
//        		System.out.println(queryTree.getDisplayName());
          }
  ...
  static private PopData<String,String> popSubquery (String argString) {
    ...
    String subquery = argString.substring(0, i+1); // split by right )
