...
public abstract class QryIop extends Qry {
  ...
  public void docIteratorTo (int index) {
      if(index < this.invertedList.df) {
          this.docIteratorIndex = index;
      }
      this.locIteratorIndex = 0;
  }
  /**
  ...
  /**
   * Get the term frequency associated with the QryIop(Term). Created by yuntiehc.
   * @param docindex It's actually not used. docIteratorIndex is used instead.
   * @param r the model this query belongs to.
   */
  public int getTf (RetrievalModel r, int docindex) {
//      return this.invertedList.getTfByDocId(docid);
  ...
  public Vector<Integer> getPosition () {
      return this.invertedList.getLoc(docIteratorIndex);
  }
  /**
 ...
 /**
  ...
  /**
   *  Returns the score according to specific model
   *  @return double a score(1/0) in unranked model or 
   */
  public double getScore(RetrievalModel r) {
      return 0;
  }
  /**
   * Return the name of the class "QryIop"
