...
public abstract class Qry {
  ...
  /**
   *  Get the score of this query. Implemented in subclass of Qry. Created by yuntiehc.
   *  @param r The retrieval model that determines what is a match
   *  @return double the score.
   */
  public double getScore(RetrievalModel r) throws IOException {return 0.0;}
  /**
   *  Get the term frequency of this term. Implemented in other getTf methods below. Created by yuntiehc.
   *  @param r The retrieval model that determines what is a match
   *  @param n documentID or document index
   *  @return integer the term frequency.
   */
  public abstract int getTf (RetrievalModel r,int n);
  
  /**
   *  Get the term frequency of this term. Usually called by QryIopterm so the actual action is implemented in the getTf in QryIop. Created by yuntiehc.
   *  @param r The retrieval model that determines what is a match
   *  @param docid documentID
   *  @return integer the term frequency.
   */
  public int getTfFirst (RetrievalModel r,int docid) {
      Qry q_0 = this.args.get(0); 
      return q_0.getTf(r, docid);

  }
  
  /**
   *  Get the maximum term frequency among arguments. Usually called by QrySopOr. Created by yuntiehc.
   *  @param r The retrieval model that determines what is a match
   *  @param n documentID or document index
   *  @return integer the term frequency.
   */
  public int getTfMax (RetrievalModel r, int docid) { //return the maximum term frequency among all the arguments for a certain document (controlled in QryEval)
      int max = 0;
      for (int i=0; i<this.args.size(); i++){
          Qry q_i = this.args.get(i); //q_0 belongs to SopScore class
          boolean done = false;
          while(!done) { //get the tf
              if (q_i.docIteratorHasMatch(r)) {
                  if (q_i.docIteratorGetMatch() == docid) {
                      int tf = q_i.getTf(r,docid);
                      if (tf > max) {
                          max = tf;
                      }    
                      done = true;
                      ...
                      q_i.docIteratorClearMatchCache();
                      done = true; //give up this term
                  }
                  else if (q_i.docIteratorGetMatch() < docid) {
                      q_i.docIteratorAdvanceTo(docid);
                  }
  ...
  /**
   *  Get the minimum term frequency among arguments. Usually called by QrySopAnd. Created by yuntiehc.
   *  @param r The retrieval model that determines what is a match
   *  @param n documentID or document index
   *  @return integer the term frequency.
   */
  public int getTfMin (RetrievalModel r, int docid) { //return the maximum term frequency among all the arguments for a certain document (controlled in QryEval)
  ...
  /**
   * Return the name of the class "Qry"
   * @return String of the name of class
   */
